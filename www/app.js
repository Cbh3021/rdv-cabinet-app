...
async function createRecurringSeries(baseData, intervalKey, count){
  const seriesId = uid();
  let currentDate = baseData.date;
  const skipped = [];
  for(let i=0; i<count; i++){
    if(i>0) currentDate = addInterval(currentDate, intervalKey);
    const conflict = appointments.find(x=>!x.deleted && x.date===currentDate && x.time===baseData.time);
    if(conflict){ skipped.push(currentDate); continue; }
    await createAppointment({...baseData, date: currentDate, seriesId});
  }
  if(skipped.length){
    alert(
      "Série créée (" + (count - skipped.length) + "/" + count + " RDV).\n\n" +
      "Créneau déjà pris, non créé pour : " + skipped.join(', ') + ".\n" +
      "Ajoute-les manuellement à un autre horaire si besoin."
    );
  }
}
...
document.getElementById('rdvList').addEventListener('click', async (e)=>{
  ...
  if(honorBtn){
    if(honorBtn.dataset.honorAction==='yes') await setHonoredStatus(honorBtn.dataset.id, true);
  ...
  if(reqBtn){
    ...
    if(reqBtn.dataset.reqAction==='accept-cancel'){
      ...
      await editAppointment(a.id, {name:a.name, phone:a.phone, date:req.requestedDate, time:req.requestedTime, reason:a.reason, notes:a.notes||'', patientRequest:{}});
...
function openModal(a){
  ...
  document.getElementById('phoneFieldHint').textContent = t('phone_hint');
  // La récurrence ne s'applique qu'à la création d'un nouveau RDV, pas à
  // la modification d'un RDV existant (pour rester simple et prévisible).
  document.getElementById('recurrenceWrap').style.display = a ? 'none' : '';
  document.getElementById('fRecurrenceInterval').value = 'none';
  document.getElementById('fRecurrenceCount').value = 3;
  document.getElementById('recurrenceCountWrap').style.display = 'none';
  document.getElementById('overlay').classList.add('open');
}
function closeModal(){ document.getElementById('overlay').classList.remove('open'); }

document.getElementById('addBtn').addEventListener('click', ()=>openModal(null));
document.getElementById('fRecurrenceInterval').addEventListener('change', (e)=>{
  document.getElementById('recurrenceCountWrap').style.display = e.target.value==='none' ? 'none' : 'block';
});
...
document.getElementById('saveBtn').addEventListener('click', async ()=>{
  ...
  const recurrenceInterval = editingId ? 'none' : document.getElementById('fRecurrenceInterval').value;
  const recurrenceCount = parseInt(document.getElementById('fRecurrenceCount').value, 10) || 1;

  if(editingId) await editAppointment(editingId, data);
  else if(recurrenceInterval !== 'none' && recurrenceCount > 1) await createRecurringSeries(data, recurrenceInterval, recurrenceCount);
...
document.getElementById('findCodePhone').addEventListener('input', ()=>{
  const digitsOnly = document.getElementById('findCodePhone').value.replace(/\D/g,'');
  const show = digitsOnly.length===8;
  ...
  if(restPast.length>0){
    ...
    if(pastMarked.length > 0){
      attendanceLine = `<div style="font-size:12.5px;color:#6b6f80;margin:-6px 0 14px;">
        📊 ${honoredCount}/${pastMarked.length} rendez-vous honorés — merci de prévenir le cabinet en cas d'empêchement.</div>`;
    }
